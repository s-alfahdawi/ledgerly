...
class TransactionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $currentCashTotal = $this->reportService->getAllTimeSummary($accountId)['current_cash'];
        ...
        return view('transactions.index', compact('transactions', 'currentCashTotal'));
    }

    /**
     * Reconcile total current cash to a user-supplied "actual cash" amount.
     *
     * Computes the total system cash across every active wallet:
     *   Σ(opening_balance) + Σ(income) − Σ(expense)
     *
     * Creates a single adjustment transaction equal to the difference:
     *   - actual > system  → income leg (positive correction)
     *   - actual < system  → expense leg (negative correction)
     *   - actual == system → no-op
     *
     * The transaction is posted to a dedicated reconciliation wallet and
     * category, both named "مطابقة", auto-created on first use. They serve
     * as a container so every adjustment is traceable.
     */
    public function cashMatch(Request $request): RedirectResponse
    {
        ...
        $summary = $this->reportService->getAllTimeSummary($accountId);
        $current = (float) $summary['current_cash'];
        $actual  = (float) $validated['actual_cash'];
        $diff    = round($actual - $current, 2);

        if (abs($diff) < 0.01) {
            return back()->with('info', 'Cash is already balanced. No adjustment created.');
        }

        $type   = $diff > 0 ? 'income' : 'expense';
        $amount = abs($diff);
        $defaultNote = \sprintf(
            'Cash adjustment — set total cash to %s (was %s)',
        ...
        DB::transaction(function () use ($accountId, $type, $amount, $validated, $defaultNote, $userId, $tz) {
            $wallet = Wallet::firstOrCreate(
                ['account_id' => $accountId, 'name' => 'مطابقة'],
                ['type' => 'other', 'opening_balance' => 0, 'is_archived' => false]
            );

            $category = Category::firstOrCreate(
                ['account_id' => $accountId, 'name' => 'مطابقة'],
                ['type' => 'both', 'color' => '#6c757d', 'is_archived' => false]
            );
        ...
        return back()->with(
            'success',
            \sprintf(
                '%s adjustment of %s created in مطابقة.',
                ucfirst($type),
